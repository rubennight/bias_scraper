...
# Schema: eventos → articulos ← fuentes
# =============================================================
...
def insertar_evento(titular: str, num_fuentes: int) -> int:
    """
    Inserta un evento detectado por clustering y retorna su id.
    Cada ejecución del pipeline genera eventos nuevos con timestamp.
    """
    conn = get_connection()
    cur  = conn.cursor()
    cur.execute("""
        INSERT INTO eventos (titular_evento, num_fuentes)
        VALUES (%s, %s)
        RETURNING id;
    """, (titular, num_fuentes))
...
def insertar_keywords(articulo_id: int, keywords: list):
    ...
    ON CONFLICT DO NOTHING evita duplicados si se re-procesa.
    """
    if not keywords:
        return
    conn = get_connection()
