...
def parsear_rss(url_feed: str) -> list:
    ...
    try:
        ...
        if resp.status_code != 200:
            log.warning(f"    [RSS] HTTP {resp.status_code}: {url_feed}")
            return []
        ...
        if usar_html_parser:
            ...
            try:
                ...
                try:
                    ...
                    log.warning(f"    [RSS] XML inválido en {url_feed}: {e}")
                    return []
        ...
        log.warning(f"    [RSS] Error parseando {url_feed}: {e}")
        return []
...
def scrape_con_newspaper(url: str, fecha_rss=None) -> dict | None:
    ...
    try:
        ...
        log.warning(f"    [Newspaper3k] Error en {url}: {e}")
        return None
...
def scrape_con_playwright(url: str, fecha_rss=None) -> dict | None:
    ...
    try:
        ...
        log.warning(f"    [Playwright] Error en {url}: {e}")
        return None
